...
def save_predict_results():
    ...
    ori_df = pd.read_csv('refined_with_script_data.csv')

    ori_list = ori_df['SMILE'].tolist()
    frames = []
    gen_mols = []
    gen_fps = []
    for i in [1,2,3]:
        gen_df = pd.read_csv('Modell_'+str(i)+'_unique.csv', header=None)
    ...
    unique_df.to_csv('Unique_model'+str(i)+'.csv', index=False)
    promising_df.to_csv('Promsining_model'+str(i)+'.csv', index=False)
