...
class Vocabulary(object):
    ...
    def encode(self, char_list):
        """Takes a list of chars (eg '[NH]' and encodes to array of indices)"""
    ...
    def tokenize(self, smiles):
        """Takes a SMILES and return a list of chars/tokens"""
        regex = '(\[[^\[\]]{1,6}\])'
...
class MolData(Dataset):
    ...
    Returns:
        A custom PyTorch dataset for training the Piror
    """
...
class Experience(object):
    """Class for prioritized experience replay that remembers the highest scored sequences seen
       and samples from them with probabilities relative to their scores."""
    ...
    def sample(self, n):
        """Sample a batch size of n experience"""
    ...
    def initiate_from(self, fname, scoring_function, Prior):
...
def tokenize(smiles):
    """Takes a SMILES string and returns a list of tokens.
       This will swap CL and Br to L and R and treat [xx] as one token"""
    regex = '(\[[^\[\]]{1,6}\])'
    smiles = replace_halogen(smiles)
    char_list = re.split(regex, smiles)
    tokenized = []
...
def canonicalize_smiles_from(fname):
    """Reads a SMILE file and returns a list of RDKIT SMILES"""
    with open(fname, 'r') as f:
        smiles_list = []
...
def filter_file_on_chars(smiles_fname, voc_fname):
    """Filters SMILES file using a vocabulary file.
       Only SMILES containing nothing but the characters in the vocabulary will be retained."""
    smiles = []
